...
def RQ1_results_all_random(A_ISSUES, B_ISSUES, A_ISSUES_EMBEDDING, B_ISSUES_EMBEDDING, B_A_ISSUES_MATCHING):
    ...
    DataLoader.save_embeddings(Const.FIREFOX_BRAVE_info, wi_sg_result_data)    

    indexes = np.arange(A_issues.shape[0])
    np.random.shuffle(indexes)
    A_issues = A_issues.loc[indexes[:3000]]
    ...
    for i, row in A_issues.iterrows():
        ...
        wi_sg_result_list_columns = ['firefox_issue_id', 'firefox_issue_text', 'firefox_issue_body', 'firefox_create_time', \
            'distance', 'relevant_issue', 'brave_issue_id', 'brave_issue_text', 'brave_issue_body', 'brave_create_time']
        for issue_id, distance in B_issue_distances[:1]:
            if distance >0.91:
                wi_sg_result_list_element = []
                print(B_issues.loc[B_issues['id'] == issue_id]['id'].values[0])
                B_issue_id = B_issues.loc[B_issues['id'] == issue_id]['id'].values[0]
                B_issue_text = B_issues.loc[B_issues['id'] == issue_id]['title'].values[0]
                B_issue_body = B_issues.loc[B_issues['id'] == issue_id]['body'].values[0]
                B_create_time = B_issues.loc[B_issues['id'] == issue_id]['created_at'].values[0]
...
if __name__ == "__main__":
    ...
    A_ISSUES = Const.FIREFOX_ISSUES_1
    B_ISSUES = Const.BRAVE_ISSUES_1
    A_ISSUES_EMBEDDING = Const.FIREFOX_EMBEDDINGS_1
    B_ISSUES_EMBEDDING = Const.BRAVE_EMBEDDINGS_1
    B_A_ISSUES_MATCHING = Const.FIREFOX_BRAVE_ISSUES_MATCHING_random
    RQ1_results_all_random(A_ISSUES, B_ISSUES, A_ISSUES_EMBEDDING, B_ISSUES_EMBEDDING, B_A_ISSUES_MATCHING)
