...
def RQ1_results_all_random(A_ISSUES, B_REVIEWS, A_ISSUES_EMBEDDING, B_REVIEWS_EMBEDDING, B_A_ISSUES_MATCHING):
    ...
    # load signal issues & telegram reviews
    A_issues = DataLoader.load_df_compressed(A_ISSUES)
    B_reviews = DataLoader.load_df_compressed(B_REVIEWS)
    ...
    for i, row in A_issues.iterrows():
        ...
        #ts_sg_result_list_element['telegram', issue_id, issue_text, create_time]
        B_issue_distances = MatchTest.get_nearest_issues(A_issue_text, B_review_embeddings, 3)
        ...
        for issue_id, distance in B_issue_distances[:1]:
            if distance >0.91:
                wi_sg_result_list_element = []
                B_issue_id = B_reviews.loc[B_reviews['reviewId'] == issue_id]['reviewId'].values[0]
                B_issue_text = B_reviews.loc[B_reviews['reviewId'] == issue_id]['content'].values[0]
                B_create_time = B_reviews.loc[B_reviews['reviewId'] == issue_id]['at'].values[0]
...
if __name__ == "__main__":
    ...
    A_ISSUES = Const.BRAVE_ISSUES_
    B_REVIEWS = Const.FIREFOX_REVIEWS_TEST
    A_ISSUES_EMBEDDING = Const.BRAVE_EMBEDDINGS_TEST
    B_REVIEWS_EMBEDDING = Const.FIREFOX_REVIEW_EMBEDDINGS_TEST
    B_A_ISSUES_MATCHING = Const.BRAVE_FIREFOX_MATCHING_result
    RQ1_results_all_random(A_ISSUES, B_REVIEWS, A_ISSUES_EMBEDDING, B_REVIEWS_EMBEDDING, B_A_ISSUES_MATCHING)
